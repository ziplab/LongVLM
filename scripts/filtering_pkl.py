...
def parse_args():
    ...
    parser.add_argument("--input_json_file", default="datasets/anet/VideoInstruct_Dataset.json", 
    ...
    parser.add_argument("--clip_feature_path", required=False, default="datasets/anet/anet_clip_part_train",
...
def main():
    ...
    for i, content in enumerate(input_json_contents):
        ...
        if valid:
            output_content = {'id': content['video_id'], 'video': f"{content['video_id']}", 'conversations': []}
            # This is critical
            if i % 2 == 0:
